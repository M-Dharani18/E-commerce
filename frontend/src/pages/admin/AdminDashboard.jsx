...
//       icon:<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>, soon:true },
...
//   useEffect(() => { if (["/admin","/admin/","/admin/dashboard"].includes(location.pathname)) fetchStats(); }, [location]);

//   const fetchStats = async () => {
//     try {
//       const [cats, prods] = await Promise.allSettled([adminAPI.getAllCategories(), adminAPI.getAllProducts(0,1000)]);
...
//         s.totalProducts=list.length; s.featuredProducts=list.filter(p=>p.isFeatured).length;
//         s.activeProducts=list.filter(p=>p.isActive).length; s.inactiveProducts=list.filter(p=>!p.isActive).length;
...
//   const currentLabel = ["/admin","/admin/","/admin/dashboard"].includes(location.pathname) ? "Dashboard"
...
//       <div style={{ fontFamily:SANS, background:T.bg, minHeight:"100vh", display:"flex", color:T.text, transition:"background 0.3s, color 0.3s" }}>
...
//                 overflowY:"auto", display:"flex", flexDirection:"column", transition:"background 0.3s" }}>
...
//                     <p style={{ fontSize:13, fontWeight:600, color:T.text, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{user.name||"Admin"}</p>
//                     <p style={{ fontSize:11, color:T.muted, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{user.email||""}</p>
//                   </div>
//                 </div>
...
//                     cursor:"pointer", display:"flex", alignItems:"center", gap:8, fontFamily:SANS, transition:"all 0.15s" }}
...
//         {/* ── MAIN ── */}
//         <div style={{ flex:1, marginLeft:sidebarOpen?240:0, transition:"margin 0.25s", display:"flex", flexDirection:"column", minWidth:0 }}>
...
//                 <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ color:T.dim }}><path d="M9 18l6-6-6-6"/></svg>
...
//                 <div style={{ width:6, height:6, borderRadius:"50%", background:T.green, animation:"pulse-green 2s infinite" }}/>
...
//                   background:theme==="dark"?"rgba(212,175,55,0.06)":"rgba(128,0,0,0.05)", transition:"all 0.15s" }}
...
//               <Route path="/invoices"   element={<ComingSoon title="GST Invoices"    desc="Auto-generate and download tax invoices as PDF" icon="invoices"/>} />
//               <Route path="/transactions" element={<ComingSoon title="Transactions" desc="Monitor UPI, COD and refund activity" icon="payments"/>} />
//               <Route path="/customers"  element={<ComingSoon title="Customers"       desc="View profiles, order history and account details" icon="customers"/>} />
//               <Route path="/reviews"    element={<ComingSoon title="Reviews & Ratings" desc="Moderate feedback and respond to customers" icon="reviews"/>} />
//             </Routes>
//           </main>
//         </div>
...
//           <p style={{ fontSize:14, color:T.muted }}>{new Date().toLocaleDateString("en-IN",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</p>
//         </div>
//         <div style={{ display:"flex", gap:10 }}>
//           <Link to="/admin/categories" style={{ padding:"9px 18px", textDecoration:"none", borderRadius:8, fontSize:13, fontWeight:600,
//             background:theme==="dark"?"rgba(212,175,55,0.08)":"rgba(128,0,0,0.06)",
//             border:`1px solid ${T.borderHi}`, color:theme==="dark"?T.gold:T.maroon }}>+ Category</Link>
//           <Link to="/admin/products"   style={{ padding:"9px 18px", textDecoration:"none", borderRadius:8, fontSize:13, fontWeight:600,
//             background:T.maroon, color:"#FFFFF0" }}>+ Product</Link>
//         </div>
//       </motion.div>
...
//               padding:"18px 16px", position:"relative", overflow:"hidden", boxShadow:T.shadow,
...
//             <div style={{ width:36, height:36, borderRadius:9, background:`${c.color}${theme==="dark"?"18":"14"}`,
//               display:"flex", alignItems:"center", justifyContent:"center", color:c.color, marginBottom:14 }}>{c.icon}</div>
//             <p style={{ fontFamily:SERIF, fontSize:28, fontWeight:700, color:T.text, lineHeight:1, marginBottom:6 }}><Counter to={c.value}/></p>
...
//       {/* Health gauges + quick actions */}
//       <div style={{ display:"grid", gridTemplateColumns:"260px 1fr", gap:16 }}>
//         <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.45}}
//           style={{ background:T.card, border:`1px solid ${T.statBorder}`, borderRadius:14, padding:"22px 20px", boxShadow:T.shadow }}>
...
//           style={{ background:T.card, border:`1px solid ${T.statBorder}`, borderRadius:14, padding:"22px", boxShadow:T.shadow }}>
...
//   // Mock data — replace with real API calls when order backend is ready
//   useEffect(() => {
//     setTimeout(() => {
//       setOrders([
//         { id:"ORD-001", customer:"Priya Raman", phone:"9876543210", date:"2026-03-07", status:"placed",    items:2, total:4800, payment:"UPI",     address:"12 MG Road, Chennai" },
//         { id:"ORD-002", customer:"Kavitha S",   phone:"9123456789", date:"2026-03-07", status:"confirmed", items:1, total:8500, payment:"COD",     address:"45 Anna Nagar, Chennai" },
//         { id:"ORD-003", customer:"Meena P",     phone:"9988776655", date:"2026-03-06", status:"shipped",   items:3, total:12400,payment:"UPI",     address:"78 T Nagar, Chennai" },
//         { id:"ORD-004", customer:"Saranya M",   phone:"9112233445", date:"2026-03-06", status:"delivered", items:1, total:6200, payment:"PhonePe", address:"23 Adyar, Chennai" },
//         { id:"ORD-005", customer:"Lakshmi K",   phone:"9090909090", date:"2026-03-05", status:"packed",    items:2, total:9100, payment:"COD",     address:"56 Velachery, Chennai" },
//         { id:"ORD-006", customer:"Divya R",     phone:"9876543211", date:"2026-03-05", status:"cancelled", items:1, total:3200, payment:"UPI",     address:"34 Pallavaram, Chennai" },
//         { id:"ORD-007", customer:"Nithya S",    phone:"9123456780", date:"2026-03-04", status:"delivered", items:4, total:18600,payment:"Paytm",   address:"89 Perambur, Chennai" },
//         { id:"ORD-008", customer:"Ranjitha V",  phone:"9988776654", date:"2026-03-04", status:"placed",    items:1, total:5400, payment:"COD",     address:"12 Sholinganallur, Chennai" },
//       ]);
//       setLoading(false);
//     }, 600);
//   }, []);
...
//     // Replace with real API: await adminAPI.updateOrderStatus(orderId, newStatus)
//     await new Promise(r => setTimeout(r, 500));
//     setOrders(prev => prev.map(o => o.id===orderId ? {...o, status:newStatus} : o));
//     setUpdatingId(null);
//     if (selectedOrder?.id===orderId) setSelected(prev => ({...prev, status:newStatus}));
//   };

//   const iStyle = {
//     padding:"9px 14px", background:T.inputBg, border:`1px solid ${T.inputBorder}`,
//     borderRadius:8, color:T.text, fontSize:13, outline:"none", fontFamily:"'DM Sans',sans-serif"
//   };
...
//   const counts = { all:orders.length, placed:0, confirmed:0, packed:0, shipped:0, delivered:0, cancelled:0 };
...
//   return (
//     <div style={{ display:"flex", gap:20, height:"calc(100vh - 120px)", fontFamily:"'DM Sans',sans-serif" }}>
...
//           <div>
//             <h1 style={{ fontFamily:SERIF, fontSize:28, fontWeight:700, color:T.text, marginBottom:4 }}>Order Management</h1>
//             <p style={{ fontSize:14, color:T.muted }}>{orders.length} total orders • {counts.placed} new</p>
//           </div>
//           <div style={{ display:"flex", gap:8 }}>
//             {[
//               {label:"Pending",count:counts.placed+counts.confirmed+counts.packed, color:T.amber, bg:T.amberBg, bd:T.amberBorder},
//               {label:"In Transit",count:counts.shipped, color:T.blue, bg:T.blueBg, bd:T.blueBorder},
//               {label:"Delivered",count:counts.delivered, color:T.green, bg:T.greenBg, bd:T.greenBorder},
//             ].map((s,i) => (
//               <div key={i} style={{ padding:"8px 14px", borderRadius:8, background:s.bg, border:`1px solid ${s.bd}` }}>
//                 <span style={{ fontFamily:SERIF, fontSize:20, fontWeight:700, color:s.color }}>{s.count}</span>
...
//         {/* Toolbar */}
//         <div style={{ display:"flex", gap:10, padding:"12px 14px", background:T.card,
...
//             </svg>
//             <input placeholder="Order ID, customer, phone…" value={search} onChange={e=>setSearch(e.target.value)}
...
//                 style={{ padding:"8px 14px", borderRadius:7, border:`1px solid ${filter===f.key?T.borderHi:T.border}`,
//                   background:filter===f.key?(theme==="dark"?"rgba(212,175,55,0.1)":"rgba(128,0,0,0.07)"):"transparent",
...
//                 {f.label}{f.key!=="all"&&counts[f.key]>0&&<span style={{ marginLeft:5, padding:"1px 5px", borderRadius:10,
//                   background:theme==="dark"?"rgba(255,255,255,0.1)":"rgba(0,0,0,0.08)", fontSize:10 }}>{counts[f.key]}</span>}
//               </button>
//             ))}
//           </div>
//         </div>
...
//                     background:theme==="dark"?"rgba(255,255,255,0.02)":T.cardAlt, whiteSpace:"nowrap" }}>{h}</th>
//                 ))}
//               </tr>
//             </thead>
//             <tbody>
...
//                   <div style={{ height:18, background:T.statBorder, borderRadius:4, animation:"fadeInOut 1.4s infinite" }}/>
//                 </td></tr>
//               )) : filtered.length===0 ? (
//                 <tr><td colSpan={8} style={{ padding:"48px 16px", textAlign:"center", color:T.muted }}>No orders found</td></tr>
//               ) : filtered.map((o, i) => (
//                 <tr key={o.id} onClick={()=>setSelected(o)} className="ord-row"
//                   style={{ borderBottom:`1px solid ${T.tableBorder}`, cursor:"pointer",
//                     background:selectedOrder?.id===o.id?(theme==="dark"?"rgba(212,175,55,0.06)":"rgba(128,0,0,0.03)"):"transparent",
//                     transition:"background 0.12s" }}>
//                   <td style={{ padding:"13px 16px", fontSize:13, fontWeight:700, color:theme==="dark"?T.gold:T.maroon }}>{o.id}</td>
//                   <td style={{ padding:"13px 16px" }}>
//                     <div style={{ fontSize:13, fontWeight:600, color:T.text }}>{o.customer}</div>
//                     <div style={{ fontSize:11, color:T.muted }}>{o.phone}</div>
//                   </td>
//                   <td style={{ padding:"13px 16px", fontSize:12, color:T.muted }}>{new Date(o.date).toLocaleDateString("en-IN")}</td>
//                   <td style={{ padding:"13px 16px", fontSize:13, color:T.text }}>{o.items} item{o.items>1?"s":""}</td>
//                   <td style={{ padding:"13px 16px", fontFamily:SERIF, fontSize:14, fontWeight:700, color:T.text }}>₹{o.total.toLocaleString("en-IN")}</td>
//                   <td style={{ padding:"13px 16px" }}>
//                     <span style={{ padding:"3px 8px", borderRadius:6, fontSize:11, fontWeight:600,
//                       background:o.payment==="COD"?T.amberBg:T.blueBg,
//                       color:o.payment==="COD"?T.amber:T.blue,
//                       border:`1px solid ${o.payment==="COD"?T.amberBorder:T.blueBorder}` }}>{o.payment}</span>
//                   </td>
//                   <td style={{ padding:"13px 16px" }}><StatusBadge status={o.status}/></td>
//                   <td style={{ padding:"13px 16px" }}>
//                     <button onClick={e=>{e.stopPropagation(); setSelected(o);}}
//                       style={{ padding:"6px 12px", background:theme==="dark"?"rgba(212,175,55,0.08)":"rgba(128,0,0,0.06)",
//                         border:`1px solid ${T.borderHi}`, color:theme==="dark"?T.gold:T.maroon,
//                         cursor:"pointer", fontSize:12, fontWeight:600, borderRadius:6, fontFamily:"'DM Sans',sans-serif" }}>
//                       Manage
//                     </button>
//                   </td>
//                 </tr>
...
//             style={{ width:340, flexShrink:0, background:T.card, border:`1px solid ${T.border}`,
//               borderRadius:14, padding:"22px", overflowY:"auto", boxShadow:T.shadowMd,
//               display:"flex", flexDirection:"column", gap:20 }}>

//             {/* Header */}
//             <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between" }}>
//               <div>
//                 <p style={{ fontSize:11, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:T.muted, marginBottom:4 }}>Order Details</p>
//                 <h2 style={{ fontFamily:SERIF, fontSize:22, fontWeight:700, color:T.text }}>{selectedOrder.id}</h2>
//               </div>
//               <button onClick={()=>setSelected(null)}
//                 style={{ width:30, height:30, borderRadius:7, background:T.hoverBg, border:`1px solid ${T.border}`,
...
//               <span style={{ fontSize:12, color:T.muted }}>{new Date(selectedOrder.date).toLocaleDateString("en-IN",{day:"numeric",month:"long",year:"numeric"})}</span>
//             </div>

//             {/* Timeline */}
//             <div style={{ padding:"16px", background:T.cardAlt||T.hoverBg, borderRadius:10, border:`1px solid ${T.border}` }}>
//               <p style={{ fontSize:11, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:T.muted, marginBottom:14 }}>Order Timeline</p>
//               {STATUS_FLOW.map((s, i) => {
//                 const currentIdx = STATUS_FLOW.indexOf(selectedOrder.status);
//                 const done = i <= currentIdx && selectedOrder.status !== "cancelled";
//                 const isCurrent = i === currentIdx && selectedOrder.status !== "cancelled";
//                 return (
//                   <div key={s} style={{ display:"flex", alignItems:"center", gap:10, marginBottom: i < STATUS_FLOW.length-1 ? 12 : 0, position:"relative" }}>
//                     {i < STATUS_FLOW.length-1 && (
//                       <div style={{ position:"absolute", left:7, top:18, width:2, height:16,
//                         background:done&&!isCurrent?T.green:"rgba(128,128,128,0.2)" }}/>
//                     )}
//                     <div style={{ width:16, height:16, borderRadius:"50%", flexShrink:0, zIndex:1,
//                       background:done?T.green:(isCurrent?"transparent":"rgba(128,128,128,0.15)"),
//                       border:`2px solid ${done||isCurrent?T.green:"rgba(128,128,128,0.2)"}`,
//                       display:"flex", alignItems:"center", justifyContent:"center" }}>
//                       {done && !isCurrent && <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>}
//                       {isCurrent && <div style={{ width:6, height:6, borderRadius:"50%", background:T.green }}/>}
//                     </div>
//                     <span style={{ fontSize:13, fontWeight:isCurrent?700:400,
//                       color:done||isCurrent?T.text:T.dim, textTransform:"capitalize" }}>{s}</span>
//                   </div>
//                 );
//               })}
...
//                     border:`2px solid ${T.danger}`, display:"flex", alignItems:"center", justifyContent:"center" }}>
//                     <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke={T.danger} strokeWidth="3"><path d="M18 6 6 18M6 6l12 12"/></svg>
//                   </div>
...
//             {/* Customer info */}
//             <div style={{ padding:"14px", background:T.cardAlt||T.hoverBg, borderRadius:10, border:`1px solid ${T.border}` }}>
//               <p style={{ fontSize:11, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:T.muted, marginBottom:12 }}>Customer</p>
//               <p style={{ fontSize:14, fontWeight:600, color:T.text, marginBottom:4 }}>{selectedOrder.customer}</p>
//               <p style={{ fontSize:13, color:T.muted, marginBottom:4 }}>📞 {selectedOrder.phone}</p>
//               <p style={{ fontSize:13, color:T.muted }}>📍 {selectedOrder.address}</p>
//             </div>

//             {/* Order summary */}
//             <div style={{ padding:"14px", background:T.cardAlt||T.hoverBg, borderRadius:10, border:`1px solid ${T.border}` }}>
//               <p style={{ fontSize:11, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:T.muted, marginBottom:12 }}>Order Summary</p>
//               {[
//                 { label:`${selectedOrder.items} item(s) subtotal`, value:`₹${selectedOrder.total.toLocaleString("en-IN")}` },
//                 { label:"Delivery charges",          value:selectedOrder.total >= 999 ? "Free" : "₹49" },
//                 { label:"GST (5%)",                  value:`₹${Math.round(selectedOrder.total*0.05).toLocaleString("en-IN")}` },
//                 { label:"Grand Total",               value:`₹${(selectedOrder.total + (selectedOrder.total>=999?0:49) + Math.round(selectedOrder.total*0.05)).toLocaleString("en-IN")}`, bold:true },
//               ].map((row, i) => (
//                 <div key={i} style={{ display:"flex", justifyContent:"space-between", alignItems:"center",
//                   marginBottom:i<3?8:0, paddingTop:i===3?8:0, borderTop:i===3?`1px solid ${T.border}`:"none" }}>
//                   <span style={{ fontSize:13, color:row.bold?T.text:T.muted, fontWeight:row.bold?700:400 }}>{row.label}</span>
//                   <span style={{ fontFamily:row.bold?SERIF:"inherit", fontSize:row.bold?16:13, fontWeight:700, color:row.bold?T.text:T.muted }}>{row.value}</span>
//                 </div>
//               ))}
//             </div>
...
//               <div>
//                 <p style={{ fontSize:11, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color:T.muted, marginBottom:10 }}>Update Status</p>
//                 <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
//                   {STATUS_FLOW.filter((_, i) => i > STATUS_FLOW.indexOf(selectedOrder.status)).map(nextStatus => (
//                     <button key={nextStatus} onClick={()=>updateStatus(selectedOrder.id, nextStatus)}
//                       disabled={updatingId===selectedOrder.id}
//                       style={{ padding:"11px", background:T.greenBg, border:`1px solid ${T.greenBorder}`,
//                         color:T.green, cursor:updatingId===selectedOrder.id?"wait":"pointer",
//                         fontSize:13, fontWeight:700, borderRadius:8, fontFamily:"'DM Sans',sans-serif",
//                         textTransform:"capitalize", transition:"all 0.15s",
//                         opacity:updatingId===selectedOrder.id?0.6:1 }}>
//                       {updatingId===selectedOrder.id?"Updating…":`Mark as ${nextStatus} →`}
//                     </button>
//                   ))}
//                   {selectedOrder.status !== "cancelled" && (
//                     <button onClick={()=>updateStatus(selectedOrder.id, "cancelled")}
//                       style={{ padding:"11px", background:T.dangerBg, border:`1px solid ${T.dangerBorder}`,
//                         color:T.danger, cursor:"pointer", fontSize:13, fontWeight:600, borderRadius:8,
//                         fontFamily:"'DM Sans',sans-serif", transition:"all 0.15s" }}>
//                       Cancel Order
//                     </button>
//                   )}
...
//             {/* Invoice button */}
//             <button style={{ padding:"12px", background:T.maroon, color:"#FFFFF0", border:"none",
//               cursor:"pointer", fontSize:13, fontWeight:700, borderRadius:8,
//               fontFamily:"'DM Sans',sans-serif", display:"flex", alignItems:"center", gap:8, justifyContent:"center" }}>
...
//       <motion.div initial={{opacity:0,scale:0.93}} animate={{opacity:1,scale:1}} style={{ textAlign:"center", maxWidth:380 }}>
//         <div style={{ width:80, height:80, borderRadius:20, background:"rgba(212,175,55,0.07)",
...
//         <Link to="/admin/dashboard" style={{ display:"inline-block", padding:"11px 26px",
//           background:T.maroon, color:"#FFFFF0", textDecoration:"none", borderRadius:8, fontSize:13, fontWeight:600 }}>
//           ← Back to Dashboard
//         </Link>
//       </motion.div>
//     </div>
